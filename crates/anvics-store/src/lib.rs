...
Codex launch note: the agent process must have write access to the workspace and must be able to run the Anvics command prefix above. \
...
## Delegation\n\n\
Do not spawn or delegate to additional agents unless the operator explicitly asks you to. If your harness automatically delegates anyway, pass it this file, the Anvics skill path, the task packet path, the repository path, the workspace id/path, and the same agent-run commands before it reads or edits code. Delegated agents must also run `agent enter`, use `workspace diff`, and report coordination status.\n\n\
...
mod tests {
    ...
    fn agent_prepare_writes_task_packet() {
        ...
        assert!(packet.contains("Do not spawn or delegate to additional agents"));
