...
Implementation should use Rust for the local daemon, storage, workspace overlays, VFS, indexing, sync, API, CLI, and legacy Git adapter. Core Anvics crates must remain Git-free; Git belongs only in `anvics-legacy-git`. MVP 0 should prove the local work-thread/review loop before implementing the full platform roadmap.
