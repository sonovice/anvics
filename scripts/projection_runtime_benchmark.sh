...
populate_repo() {
  local repo="$1"
  mkdir -p "$repo/src/module_01" "$repo/src/module_02" "$repo/docs" "$repo/fixtures/medium" "$repo/target/cache"
...
run_case() {
  ...
  run_output="$(anvics_for_repo "$repo" command run \
    ...
    -- \
    sh -c "printf 'changed through $projection\n' > src/module_01/file_001.txt && printf 'new through $projection\n' > src/module_03/added_from_projection.txt && rm src/module_02/file_002.txt" 2>&1)"
  status=$?
  set -e
  ...
  accept="$(anvics_for_repo "$repo" agent accept \
    ...
    --output "$patch" \
    -- \
    sh -c "grep 'changed through $projection' src/module_01/file_001.txt && test -f src/module_03/added_from_projection.txt && test ! -e src/module_02/file_002.txt")"
